...
lme4::lmer(lf ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(vlf ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(sdnn ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(rmssd ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(mean_rr ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(pns ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(sns ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
lme4::lmer(stress ~ time * frail_total + (1|id), data = datos_modelo, REML = TRUE) |> parameters::parameters() 
  
# ID y tiempo en efectos fijos (?) duda existencial
#calcular los deltas entre pre-peri , peri-post , pre-post

# quizas añadir variables del TM6
